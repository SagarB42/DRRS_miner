...
void sha256d(hls::stream<ap_axiu<640,0,0,0> >& input, hls::stream<ap_axiu<256,0,0,0> >& output) {
	...
	ap_axiu<640,0,0,0> axiuIn = input.read();
	myuint8 input_data[80];
	for (int i = 0; i < 80; i++) {
	...
	Transfrom: for (int t = 0; t < 2; t++) {
		myuint32 a, b, c, d, e, f, g, h, i, j, t1, t2, m[64];
	...
	ap_axiu<256,0,0,0> axiuOut;
	for (int i = 0; i < 8; ++i) {
		axiuOut.data.range((i + 1) * 32 - 1, i * 32) = state[i];
	}
	axiuOut.last = axiuIn.last;
	output.write(axiuOut);
}
